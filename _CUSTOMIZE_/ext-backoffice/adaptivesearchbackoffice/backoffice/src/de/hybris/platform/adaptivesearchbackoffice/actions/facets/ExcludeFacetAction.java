...
public class ExcludeFacetAction extends AbstractComponentWidgetAdapterAware implements CockpitAction<AbstractEditorData, Object>
{
	...
	public ActionResult<Object> perform(final ActionContext<AbstractEditorData> ctx)
	{
		...
		{
			asConfigurationService.moveConfiguration(searchConfiguration, PROMOTEDFACETS, EXCLUDEDFACETS, facet.getUid());
		}
		else if (facet instanceof AsFacetModel)
		{
			asConfigurationService.moveConfiguration(searchConfiguration, FACETS, EXCLUDEDFACETS, facet.getUid());
		}
		else
		{
	...
	public boolean canPerform(final ActionContext<AbstractEditorData> ctx)
	{
		...
		if (!(model instanceof AsPromotedFacetModel || model instanceof AsFacetModel))
		{
			return false;
		}
