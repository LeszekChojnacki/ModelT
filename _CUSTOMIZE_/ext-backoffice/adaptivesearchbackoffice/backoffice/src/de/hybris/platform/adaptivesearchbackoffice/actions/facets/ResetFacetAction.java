...
public class ResetFacetAction extends AbstractComponentWidgetAdapterAware implements CockpitAction<AbstractEditorData, Object>
{
	...
	public ActionResult<Object> perform(final ActionContext<AbstractEditorData> ctx)
	{
		...
		{
			asConfigurationService.moveConfiguration(searchConfiguration, PROMOTEDFACETS, FACETS, facet.getUid());
		}
		else if (facet instanceof AsExcludedFacetModel)
		{
			asConfigurationService.moveConfiguration(searchConfiguration, EXCLUDEDFACETS, FACETS, facet.getUid());
		}
		else
		{
	...
	public boolean canPerform(final ActionContext<AbstractEditorData> ctx)
	{
		...
		if (!(model instanceof AsPromotedFacetModel || model instanceof AsExcludedFacetModel))
		{
			return false;
		}
