...
/**
 * Action that moves a promoted item down
 */
public class RankItemDownAction extends AbstractResultAction
{
	...
	public ActionResult<Object> perform(final ActionContext<DocumentModel> ctx)
	{
		...
		asConfigurationService.rerankConfiguration(searchConfiguration,
				AbstractAsConfigurableSearchConfigurationModel.PROMOTEDITEMS, data.getPromotedItemUid(), +1);
	...
	public boolean canPerform(final ActionContext<DocumentModel> ctx)
	{
		...
		return searchConfiguration.getPromotedItems() != null && !searchConfiguration.getPromotedItems().isEmpty()
				&& !data.getPromotedItemUid()
						.equals(searchConfiguration.getPromotedItems().get(searchConfiguration.getPromotedItems().size() - 1).getUid())
