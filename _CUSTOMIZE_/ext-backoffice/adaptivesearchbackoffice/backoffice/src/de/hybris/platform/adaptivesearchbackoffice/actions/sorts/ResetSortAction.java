...
public class ResetSortAction extends AbstractComponentWidgetAdapterAware implements CockpitAction<AbstractEditorData, Object>
{
	...
	public ActionResult<Object> perform(final ActionContext<AbstractEditorData> ctx)
	{
		...
		{
			asConfigurationService.moveConfiguration(searchConfiguration, PROMOTEDSORTS, SORTS, sort.getUid());
		}
		else if (sort instanceof AsExcludedSortModel)
		{
			asConfigurationService.moveConfiguration(searchConfiguration, EXCLUDEDSORTS, SORTS, sort.getUid());
		}
		else
		{
	...
	public boolean canPerform(final ActionContext<AbstractEditorData> ctx)
	{
		...
		if (!(model instanceof AsPromotedSortModel || model instanceof AsExcludedSortModel))
		{
			return false;
		}
