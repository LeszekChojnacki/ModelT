/*
 * [y] hybris Platform
 *
 * Copyright (c) 2018 SAP SE or an SAP affiliate company.  All rights reserved.
 *
 * This software is the confidential and proprietary information of SAP
 * ("Confidential Information"). You shall not disclose such Confidential
 * Information and shall use it only in accordance with the terms of the
 * license agreement you entered into with SAP.
 */
package com.hybris.backoffice.catalogversioneventhandling;

/**
 * Service responsible for registering and informing about catalog versions changes
 */
public interface AvailableCatalogVersionsTag
{
	/**
	 * @return current catalog versions tag. Tag is changed after changes in catalog versions
	 */
	Object getTag();

	/**
	 * method should be invoked after creation or removal catalog version
	 */
	void refresh();
}
