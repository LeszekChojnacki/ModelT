...
/**
 * This action specifies that the payment void operation has been handled manually. It proceeds with the regular cancellation
...
public class ManualPaymentVoidAction extends AbstractComponentWidgetAdapterAware implements CockpitAction<OrderModel, OrderModel>
{
	...
	public ActionResult<OrderModel> perform(final ActionContext<OrderModel> actionContext)
	{
		...
		if (actionContext != null && actionContext.getData() != null)
		{
			...
			notificationService.notifyUser(notificationService.getWidgetNotificationSource(actionContext), CustomersupportbackofficeConstants.NOTIFICATION_TYPE, NotificationEvent.Level.SUCCESS,
					actionContext.getLabel(MANUAL_PAYMENT_VOID_SUCCESS));
	...
	/**
	 * Executes the manual payment void release operation by calling the {@link BusinessProcessService}.
	 *
	 * @param order
	 * 		the {@link OrderModel} to be released
	 */
	protected void executeManualPaymentVoidOperation(final OrderModel order)
	{
		order.getOrderProcess().stream()
				...
				filteredProcess -> getBusinessProcessService()
						.triggerEvent(filteredProcess.getCode() + "_" + MANUAL_VOID_PAYMENT_EVENT));
