...
/**
 * This action specifies that the tax commit operation has been handled manually. It proceeds with the regular order fulfillment
...
public class ManualTaxCommitAction extends AbstractComponentWidgetAdapterAware implements CockpitAction<OrderModel, OrderModel>
{
	...
	public ActionResult<OrderModel> perform(final ActionContext<OrderModel> actionContext)
	{
		...
		if (actionContext != null && actionContext.getData() != null)
		{
			...
			notificationService.notifyUser(notificationService.getWidgetNotificationSource(actionContext), CustomersupportbackofficeConstants.NOTIFICATION_TYPE, NotificationEvent.Level.SUCCESS,
					actionContext.getLabel(MANUAL_TAX_COMMIT_SUCCESS));
		}
	...
	/**
	 * Executes the manual tax commit release operation by calling the {@link BusinessProcessService}.
	 *
	 * @param order
	 * 		the {@link OrderModel} to be released
	 */
	protected void executeManualTaxCommitOperation(final OrderModel order)
	{
		order.getOrderProcess().stream()
				...
				filteredProcess -> getBusinessProcessService()
						.triggerEvent(filteredProcess.getCode() + "_" + MANUAL_TAX_COMMIT_EVENT));
		if (LOG.isDebugEnabled())
		{
			LOG.info(String.format("Tax Commit Manual Release completed. %s triggered.", MANUAL_TAX_COMMIT_EVENT));
		}
