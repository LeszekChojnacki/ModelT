...
/**
 * This cell renderer displays a button to release {@link ConsignmentModel} from a waiting step following a failed tax commit.
 */
public class TaxCommitManualReleaseButtonCellRenderer extends ReleaseButtonCellRenderer
		implements WidgetComponentRenderer<Listcell, ListColumn, ConsignmentModel>
{
	...
	protected void executeManualRelease(final ConsignmentModel consignmentModel)
	{
		if (canPerformOperation(consignmentModel, ConsignmentStatus.TAX_NOT_COMMITTED))
		{
			triggerBusinessProcessEvent(consignmentModel, CONSIGNMENT_ACTION_EVENT_NAME, HANDLE_MANUAL_TAX_COMMIT_CHOICE);
		}
	}
