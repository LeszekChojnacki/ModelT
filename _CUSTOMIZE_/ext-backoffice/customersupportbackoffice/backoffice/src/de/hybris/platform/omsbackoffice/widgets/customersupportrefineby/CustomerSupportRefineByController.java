...
public class CustomerSupportRefineByController extends DefaultWidgetController
{
	...
	{
		final DefaultConfigContext context = new DefaultConfigContext(CUSTOMERSUPPORTBACKOFFICE_REFINE_BY, typeCode.trim());
			...
			final SearchValue searchFieldValue, final Div searchFieldDiv)
	{
		...
		if (searchFieldValue.getUniqueValue() != null && StringUtils.isNotEmpty(searchFieldValue.getUniqueValue()))
		{
			...
			final Object possibleValue = getFlexibleSearchService().searchUnique(query);
		...
		if (getRefineByConfig() != null)
		{
			...
			if (dataType != null && fieldList != null)
			{
				fieldList.getSearchField().forEach(searchField ->
				{
					...
					getCustomerSupportFilterContainer().appendChild(searchFieldDiv);
				});
			}
		}
	}
	...
	{
		getCustomerSupportFilterContainer().setId(REFINE_BY_CONTAINER);
		while (getCustomerSupportFilterContainer().getChildren().size() > 0)
		{
			final Component comp = getCustomerSupportFilterContainer().getChildren().get(0);
	...
	protected void submitFilter(final Event event)
	{
		...
		final List<Component> refineBySections = getCustomerSupportFilterContainer().getChildren();
	...
	{
		return customersupportFilterContainer;
	}

	public void setCustomerSupportFilterContainer(final Div customersupportFilterContainer)
	{
