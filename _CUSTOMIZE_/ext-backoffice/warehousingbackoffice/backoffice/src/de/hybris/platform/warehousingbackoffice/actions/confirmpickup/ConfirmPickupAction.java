...
public class ConfirmPickupAction extends AbstractConsignmentWorkflow implements CockpitAction<ConsignmentModel, ConsignmentModel>
{
	...
	public boolean canPerform(final ActionContext<ConsignmentModel> actionContext)
	{
		...
		// A consignment is not available for pick up when it is a shipping order or when there is no quantity pending
	...
	public ActionResult<ConsignmentModel> perform(final ActionContext<ConsignmentModel> actionContext)
	{
		try
		{
			getWarehousingShippingService().confirmPickupConsignment(actionContext.getData());
		}
		...
		return getConsignmentActionResult(actionContext, SUCCESS_MESSAGE, FAILED_MESSAGE, ConsignmentStatus.PICKUP_COMPLETE);
	}
