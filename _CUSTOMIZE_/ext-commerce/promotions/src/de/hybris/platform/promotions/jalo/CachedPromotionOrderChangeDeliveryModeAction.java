...
public class CachedPromotionOrderChangeDeliveryModeAction extends GeneratedCachedPromotionOrderChangeDeliveryModeAction implements // NOSONAR
		JaloOnlyItem
{
			...
			throws JaloBusinessException
	{
		...
		if (!checkMandatoryAttribute(AbstractPromotionAction.MARKEDAPPLIED, allAttributes, missing)
				| !checkMandatoryAttribute(DELIVERYMODE, allAttributes, missing)) // NOSONAR
		{
		...
		try
		{
			final CachedPromotionOrderChangeDeliveryModeAction newOne = (CachedPromotionOrderChangeDeliveryModeAction) cl
					.newInstance();
	...
	protected AbstractPromotionAction deepClone(final SessionContext ctx)
	{
		try
		{
			...
			final ComposedType type = TypeManager.getInstance().getComposedType(PromotionOrderChangeDeliveryModeAction.class); // NOSONAR
			try //NOSONAR
			{
