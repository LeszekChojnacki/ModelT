 ...
 *
 * Get a product free when you spend at least a certain value. For example: <i>Get a free gravy boat when you spend over
 * &euro;150.00</i>. Get a free gift when your order subtotal is at least the threshold value.
 *
 */
public class OrderThresholdFreeGiftPromotion extends GeneratedOrderThresholdFreeGiftPromotion //NOSONAR
{
	...
	public List<PromotionResult> evaluate(final SessionContext ctx, final PromotionEvaluationContext promoContext)
	{
		...
		if (checkRestrictions(ctx, promoContext))
		{
			...
			if (threshold != null)
			{
				...
				// If we pass the threshold then fire, and add an action to put a free gift in the basket
				if (orderSubtotalAfterDiscounts >= threshold.doubleValue())
				{
					...
					final Product product = this.getGiftProduct(ctx); // NOSONAR
	...
	public String getResultDescription(final SessionContext ctx, final PromotionResult result, final Locale locale)
	{
		...
		if (order != null)
		{
			...
			if (threshold != null)
			{
				...
				{
					// "You got a free ProductName for spending over {1}"
					final Object[] args =
				...
				else if (result.getCouldFire(ctx))
				{
					...
					// "Spend {1} to get a free ProductName - Spend another {3} to qualify"
					final Object[] args =
					{ threshold, Helper.formatCurrencyAmount(ctx, locale, orderCurrency, threshold.doubleValue()),
